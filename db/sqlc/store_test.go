...
func TestTransferTx(t *testing.T) {
	...
	// Connect concurrent goroutines and share data without explicit locking.

	// receive errors
	errs := make(chan error)

	// receive transfertx results
	results := make(chan TransferTxResult)

	for i := 0; i < n; i++ {
		//txName := fmt.Sprintf("tx %d", i+1)
		//fmt.Println("transaction: ", txName)
		go func() {
	...
	for i := 0; i < n; i++ {
		...
		//amount, 2*amount, 3*amount,... n*amount
